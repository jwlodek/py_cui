...
dummy_widget = py_cui.widgets.Widget('1', 'Test', dummy_grid, 1,1,1,1,1,0, logger)
color_rule_A = py_cui.colors.ColorRule('@.*@',  py_cui.RED_ON_BLACK, 'contains',        'regex',    None,   False, logger)
color_rule_B = py_cui.colors.ColorRule('Test',  py_cui.RED_ON_BLACK, 'startswith',      'line',     None,   False, logger)
color_rule_C = py_cui.colors.ColorRule('Space', py_cui.RED_ON_BLACK, 'endswith',        'regex',    None,   False, logger)
color_rule_D = py_cui.colors.ColorRule('Test',  py_cui.RED_ON_BLACK, 'notstartswith',   'region',   [3,5],  False, logger)
color_rule_E = py_cui.colors.ColorRule('Test',  py_cui.RED_ON_BLACK, 'notendswith',     'line',     None,   False, logger)


def test_check_match():
    assert color_rule_A._check_match(test_string_A) == True
    assert color_rule_B._check_match(test_string_A) == False
    assert color_rule_C._check_match(test_string_A) == False
    assert color_rule_D._check_match(test_string_A) == True
    assert color_rule_E._check_match(test_string_A) == False
    
    assert color_rule_A._check_match(test_string_B) == False
    assert color_rule_B._check_match(test_string_B) == True
    assert color_rule_C._check_match(test_string_B) == True
    assert color_rule_D._check_match(test_string_B) == False
    assert color_rule_E._check_match(test_string_B) == True
    
    assert color_rule_A._check_match(test_string_C) == False
    assert color_rule_B._check_match(test_string_C) == False
    assert color_rule_C._check_match(test_string_C) == False
    assert color_rule_D._check_match(test_string_C) == True
    assert color_rule_E._check_match(test_string_C) == True
